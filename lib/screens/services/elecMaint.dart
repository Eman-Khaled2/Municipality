...
class MaintEScreen extends StatefulWidget {
  static String routeName = "/mainte";

  const MaintEScreen({Key? key, required this.userData}) : super(key: key);

  final Map<String, dynamic> userData;

  @override
  State<MaintEScreen> createState() => _MaintEScreenState();
}

class _MaintEScreenState extends State<MaintEScreen> {
  ...
  Future<int> fetchSubscriptionFee() async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'serviceName': 'Electricity maintenance', // Provide the service name
        }),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          LocaleKeys.Electricity_Subscription_Line_Maintenance.tr(),
                    ...
                    onPressed: () {
                      if (_selectedMaintenanceType == null) {
                        ...
                        showDialog(
                          context: context,
                          builder: (context) => AlertDialog(
                            title: Text('Subscription Line Maintenance Information'),
                            content: Column(
                              ...
                              children: [
                                ...
                                Divider(color: Colors.grey[300], thickness: 1),
                                Text('Type of Maintenance: $_selectedMaintenanceType'),
                                Divider(color: Colors.grey[300], thickness: 1),
                                FutureBuilder<int>(
                                  future: _subscriptionFeeFuture,
                                  builder: (context, snapshot) {
    ...
    if (response.statusCode == 200) {
      ...
      if (userData['name'] != null &&
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => MaintEScreen(userData: userDataMap),
          ),
        );
      } else {
