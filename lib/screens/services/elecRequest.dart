...
class RequestEScreen extends StatefulWidget {
  @override
  static String routeName = "/requeste";

  const RequestEScreen({Key? key, required this.userData}) : super(key: key);

  final Map<String, dynamic> userData;

  @override
  State<RequestEScreen> createState() => _RequestEScreenState();
}

class _RequestEScreenState extends State<RequestEScreen> {
  ...
  Future<int> fetchSubscriptionFee() async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'serviceName': 'Electricity request', // Provide the service name
        }),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          LocaleKeys.Electricity_Subscription_Request.tr(),
    ...
    if (response.statusCode == 200) {
      ...
      if (userData['name'] != null &&
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => RequestEScreen(userData: userDataMap),
          ),
        );
      } else {
