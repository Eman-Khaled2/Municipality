...
class StopEScreen extends StatefulWidget {
  @override
  static String routeName = "/stope";

  const StopEScreen({Key? key, required this.userData}) : super(key: key);

  final Map<String, dynamic> userData;

  @override
  State<StopEScreen> createState() => _StopEScreenState();
}

class _StopEScreenState extends State<StopEScreen> {
  ...
  Future<int> fetchSubscriptionFee() async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'serviceName': 'Electricity stop', // Provide the service name
        }),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          LocaleKeys.Stop_Subscription.tr(),
    ...
    if (response.statusCode == 200) {
      ...
      if (userData['name'] != null &&
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => StopEScreen(userData: userDataMap),
          ),
        );
      } else {
