...
class RequestFormScreen extends StatefulWidget {
  @override
  static String routeName = "/request_form";

  const RequestFormScreen({Key? key, required this.userData}) : super(key: key);

  final Map<String, dynamic> userData;

  State<RequestFormScreen> createState() => _RequestFormScreen();
}

class _RequestFormScreen extends State<RequestFormScreen> {
  ...
  Future<int> fetchSubscriptionFee() async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'serviceName': 'Sewage network connection with a facility', // Provide the service name
        }),
    ...
    if (response.statusCode == 200) {
      ...
      if (userData['name'] != null && userData['address'] != null && userData['phone_number'] != null) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => RequestFormScreen(userData: userDataMap),
          ),
        );
      } else {
