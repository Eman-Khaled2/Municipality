...
class StopWScreen extends StatefulWidget {
  @override
  static String routeName = "/stopw"; 

  const StopWScreen({Key? key, required this.userData}) : super(key: key);

  final Map<String, dynamic> userData;

  @override
  State<StopWScreen> createState() => _StopWScreenState();
}

class _StopWScreenState extends State<StopWScreen> {
  ...
  Future<int> fetchSubscriptionFee() async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'serviceName': 'Water stop', // Provide the service name
        }),
    ...
    if (response.statusCode == 200) {
      ...
      if (userData['name'] != null &&
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => StopWScreen(userData: userDataMap),
          ),
        );
      } else {
